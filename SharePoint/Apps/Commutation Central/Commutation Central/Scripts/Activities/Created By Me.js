...
function RenderActivities(currentUser) {
    GetActivities(currentUser).done(function (results) {
...
// Get activities
function GetActivities(currentUser) {
    var endpointUrl = appweburl + "/_api/web/lists/getbytitle('Activity')/Items?$select=*,Project/Title,AssignedTo/Title,Author/Title,ActivityCategory/Title&$expand=Project,AssignedTo,Author,ActivityCategory&$filter=Author/Title eq '" + currentUser + "'&$top=5000";
    return $.ajax({
        url: endpointUrl,
        type: "GET",
        headers: { "accept": "application/json;odata=verbose" }
