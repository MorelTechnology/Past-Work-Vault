(function () {
    ...
    angular
        ...
        .factory('noteDocumentItemFactory', ['baseItemFactory', '$q', '$http', 'Upload', '$uibModal', 'sharedProperties', function (baseService, $q, $http, Upload, $uibModal, sharedProperties) {
            var listEndPoint = '/_api/web/lists';
            var listName = 'Commutation Documents';
            var selectFields = '*,Note/ID,Author/Title,Editor/Title,Project/ID';
            var expandFields = "Note,File,Author,Editor,Project";
            var itemType = 'SP.Data.Commutation_x0020_DocumentsListItem';
            var contentTypeId = '0x01004DF33D31E01B48718B094465996DA54D';
            ...
            var uploadFile = function (file, projectId, noteId) {
                ...
                .then(function (result) {
                    ...
                    deferred.notify({ percentComplete: 0, action: "Creating note documents subfolder...", inProgress: true });
                    createFolder(projectId + "/Note Documents")
                    .then(function (result) {
                        deferred.notify({ percentComplete: 0, action: "Note documents subfolder created successfully", inProgress: true });
                        deferred.notify({ percentComplete: 0, action: "Uploading document...", inProgress: true });
                        baseService.digest()
                        .then(function (digest) {
                            addNew(file, projectId + "/Note Documents", digest)
                            .then(function (result) {
